...
if EMAIL_DATA_KEY not in st.session_state:
    st.session_state[EMAIL_DATA_KEY] = GEEmailData()
...
async def get_email_data(
    ...
    tasks = {
        "news_stories": news_scraper.get_n_stories_for_region("gsy", num_news),
        "business_stories": news_scraper.get_n_stories_for_region("gsy_business", num_business),
        "sports_stories": news_scraper.get_n_stories_for_region("gsy_sport", num_sports),
        "community_stories": news_scraper.get_n_stories_for_region("gsy_community", num_community),
...
with col1:
    ...
    if st.button("Fetch"):
        with st.spinner("Fetching stories"):
            email_data: GEEmailData = asyncio.run(get_email_data(
                num_news = num_news,
...
with col2:
    ...
    if len(st.session_state[EMAIL_DATA_KEY].news_stories) > 0:
        rendered_html = EmailBuilder.Gsy().render(st.session_state[EMAIL_DATA_KEY])
